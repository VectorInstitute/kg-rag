...
class GraphRAGIndexer:
    ...
    def __init__(
        ...
        self.entity_extractor = EntityRelationshipExtractor.build_default(llm=self.er_llm)
        self.entity_summarizer = EntityRelationshipDescriptionSummarizer.build_default(llm=self.es_llm)
        ...
        self.report_generator = CommunityReportGenerator.build_default(llm=self.er_llm)
        self.report_writer = CommunityReportWriter()
        
        self.communities_report_artifacts_generator = CommunitiesReportsArtifactsGenerator(
    ...
    def index_documents(self, documents: List[Document]) -> Dict[str, Any]:
        ...
        if self.verbose:
            ...
            if "text_units" in artifacts:
                print(f"Generated {len(artifacts['text_units'])} text units")
            
            if "entities" in artifacts:
                print(f"Extracted {len(artifacts['entities'])} entities")
            
            if "relations" in artifacts:
                print(f"Extracted {len(artifacts['relations'])} relations")
            
            if "communities" in artifacts:
                print(f"Detected {len(artifacts['communities'])} communities")
    ...
    def load_artifacts(self, file_path: str) -> Dict[str, Any]:
        """
